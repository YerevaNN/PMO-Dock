...
class OracleComponent(ABC):
    ...
    def __init__(self, parameters: OracleComponentParameters):
        ...
        self.lead_smiles = parameters.lead_smiles
    ...
    def calculate_reward(self, mols: np.ndarray[Mol], oracle_calls: int) -> Tuple[np.ndarray[float], np.ndarray[float]]:
        ...
        if self.name in ("geam", "hit"):
            if self.parameters.lead_smiles:
                raw_vina, qed_rewards, raw_sa, raw_similarity, aggregated_rewards = self(mols)
                return raw_vina, qed_rewards, raw_sa, raw_similarity, aggregated_rewards
            else:
