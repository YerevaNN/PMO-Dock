...
class Generator:
    ...
    def likelihood_smiles(
        self, smiles: np.ndarray[str], max_batch_size: int = 32
    ) -> torch.Tensor:
        """
        Compute NLL for each SMILES. Processes in sub-batches of max_batch_size to avoid
        CUDA OOM when many SMILES are passed (e.g. batch + replay + hallucinations).
        """
        ...
        for i in range(0, len(sequences), max_batch_size):
            ...
            padded = collate_fn(chunk)
            likelihoods.append(self.likelihood(padded))
        return torch.cat(likelihoods, dim=0)
