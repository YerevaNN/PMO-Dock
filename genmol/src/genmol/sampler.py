...
def load_model_from_path(path):
    # Load checkpoint to CPU first to avoid CUDA initialization issues
    # This is especially important when CUDA_VISIBLE_DEVICES is set
    # The issue is that PyTorch tries to restore tensors to their original device
    # during unpickling, which can trigger CUDA initialization errors
    import torch
    ...
    # Monkey-patch torch.load to force CPU loading BEFORE Lightning calls it
    # This ensures all tensors are loaded to CPU during unpickling
    original_load = torch.load
    def cpu_load(*args, **kwargs):
        # Force map_location to CPU if not already specified
    ...
    try:
        # Try loading with map_location parameter first
        try:
            model = GenMol.load_from_checkpoint(path, map_location='cpu')
        except (TypeError, ValueError):
            # If map_location is not supported, the monkey-patched torch.load will handle it
            model = GenMol.load_from_checkpoint(path)
    finally:
        # Always restore original torch.load
        torch.load = original_load
    
    # Log initial device (should be CPU after loading)
    initial_device = next(model.backbone.parameters()).device
    logger.info(f"Model loaded on device: {initial_device}")
    
    # Now move model to appropriate device after loading is complete
    # Only check CUDA after model is fully loaded to avoid initialization issues
    target_device = None
    ...
    try:
        ...
        if cuda_available:
            ...
            logger.info(f"CUDA available: {cuda_available}, GPUs: {num_gpus}, "
                       f"Current device: {current_device}, Device name: {device_name}")
            
            if 'CUDA_VISIBLE_DEVICES' in os.environ:
                logger.info(f"CUDA_VISIBLE_DEVICES={os.environ['CUDA_VISIBLE_DEVICES']}")
                # CUDA_VISIBLE_DEVICES is set, so we should use cuda:0 (which maps to the first visible GPU)
                target_device = torch.device('cuda:0')
            else:
                # No CUDA_VISIBLE_DEVICES, use cuda:0 by default
                target_device = torch.device('cuda:0')
        else:
            logger.warning("CUDA is not available. Model will remain on CPU.")
    except RuntimeError as e:
        logger.warning(f"Error checking CUDA availability: {e}. Model will remain on CPU.")
    
    # Move model to target device if CUDA is available
    if target_device is not None and cuda_available:
        try:
            logger.info(f"Moving model to device: {target_device}")
            # For Lightning modules, we need to move both the module and set the device attribute
            model = model.to(target_device)
            # Also explicitly move the backbone to ensure it's on the right device
            model.backbone = model.backbone.to(target_device)
            # Verify the model is actually on the target device
            final_device = next(model.backbone.parameters()).device
            if final_device.type == 'cuda':
                logger.info(f"Model successfully moved to GPU: {final_device}")
                logger.info(f"GPU memory allocated: {torch.cuda.memory_allocated(final_device.index) / 1024**3:.2f} GB")
                logger.info(f"GPU memory reserved: {torch.cuda.memory_reserved(final_device.index) / 1024**3:.2f} GB")
            else:
                logger.warning(f"Model is still on CPU after attempting to move to {target_device}")
        except RuntimeError as e:
            logger.error(f"Failed to move model to {target_device}: {e}. Model will remain on CPU.")
    else:
        logger.info("Model will remain on CPU (CUDA not available or not requested)")
